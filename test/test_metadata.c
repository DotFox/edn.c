...
TEST(basic_metadata_map) {
    /* ^{:test true} [1 2 3] */
    edn_result_t result = edn_read("^{:test true} [1 2 3]", 0);
    ...
    edn_result_t key_result = edn_read(":test", 0);
...
TEST(string_metadata_tag) {
    ...
    edn_result_t key_result = edn_read(":tag", 0);
    edn_value_t* value = edn_map_lookup(meta, key_result.value);
    assert(value != NULL);
...
TEST(chained_metadata) {
    ...
    edn_result_t result = edn_read("^:test ^:foo [1 2 3]", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_VECTOR);
    ...
    edn_result_t key1 = edn_read(":test", 0);
    edn_result_t key2 = edn_read(":foo", 0);
...
TEST(mixed_chained_metadata) {
    ...
    edn_result_t result = edn_read("^{:a 1} ^:b [1 2 3]", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_VECTOR);
    ...
    edn_result_t key_a = edn_read(":a", 0);
    ...
    edn_result_t key_b = edn_read(":b", 0);
...
TEST(overlapping_chained_metadata) {
    ...
    edn_result_t result = edn_read("^{:a \"outer\"} ^{:a \"inner\"} [1 2 3]", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_VECTOR);
...
TEST(metadata_on_map) {
    /* ^:test {:a 1 :b 2} */
    edn_result_t result = edn_read("^:test {:a 1 :b 2}", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
...
TEST(metadata_on_symbol) {
    /* ^:test foo */
    edn_result_t result = edn_read("^:test foo", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
...
TEST(metadata_invalid_type) {
    ...
    edn_result_t result = edn_read("^123 [1 2 3]", 0);
...
TEST(metadata_complex_map) {
    edn_result_t result = edn_read("^{:doc \"A vector\" :test true :tag Vector} [1 2 3]", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_VECTOR);
    ...
    /* Check :doc */
    edn_result_t key_doc = edn_read(":doc", 0);
...
TEST(metadata_invalid_value_type_number) {
    ...
    edn_result_t result = edn_read("^:test 123", 0);
    assert(result.error == EDN_ERROR_INVALID_SYNTAX);
    assert(result.error_message != NULL);
}
...
TEST(metadata_invalid_value_type_string) {
    ...
    edn_result_t result = edn_read("^:test \"hello\"", 0);
    assert(result.error == EDN_ERROR_INVALID_SYNTAX);
    assert(result.error_message != NULL);
}
...
TEST(metadata_invalid_value_type_nil) {
    ...
    edn_result_t result = edn_read("^:test nil", 0);
    assert(result.error == EDN_ERROR_INVALID_SYNTAX);
    assert(result.error_message != NULL);
}
...
TEST(metadata_invalid_value_type_bool) {
    ...
    edn_result_t result = edn_read("^:test true", 0);
    assert(result.error == EDN_ERROR_INVALID_SYNTAX);
    assert(result.error_message != NULL);
}
...
TEST(metadata_on_tagged_literal) {
    ...
    edn_result_t result = edn_read("^:test #inst \"2024-01-01\"", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
...
TEST(metadata_vector_param_tags) {
    ...
    edn_result_t result = edn_read("^[String long _] foo", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_SYMBOL);
    ...
    edn_result_t key = edn_read(":param-tags", 0);
...
TEST(metadata_vector_chained) {
    ...
    edn_result_t result = edn_read("^:test ^[String] foo", 0);
    assert(result.error == EDN_OK);
    assert(result.value != NULL);
    assert(edn_type(result.value) == EDN_TYPE_SYMBOL);
    ...
    /* Check :param-tags key */
    edn_result_t key1 = edn_read(":param-tags", 0);
    edn_value_t* val1 = edn_map_lookup(meta, key1.value);
    assert(val1 != NULL);
    assert(edn_type(val1) == EDN_TYPE_VECTOR);

    /* Check :test key */
    edn_result_t key2 = edn_read(":test", 0);
    edn_value_t* val2 = edn_map_lookup(meta, key2.value);
    assert(val2 != NULL);
    assert(edn_type(val2) == EDN_TYPE_BOOL);
